...
def draw_route(graph: Graph, path: list, filename: str = "route.png", title: str = "Duong di ngan nhat"):
    ...
    nx.draw_networkx_edges(
        G, pos, edge_color="lightgray", arrows=True, arrowsize=18,
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=route_nodes, node_color="tomato", node_size=950)
    nx.draw_networkx_edges(
        G, pos, edgelist=route_edges, edge_color="crimson", width=3,
        arrows=True, arrowsize=22, connectionstyle="arc3,rad=0.05",
    )
...
def draw_multi_route(graph: Graph, full_route, filename: str = "multi_route.png"):
    ...
    for index, segment in enumerate(full_route or []):
        ...
        nx.draw_networkx_nodes(
            G, pos, nodelist=[node for node in path if node in G],
            node_color=color, node_size=950,
        )
        nx.draw_networkx_edges(
            G, pos, edgelist=route_edges, edge_color=color, width=3,
            arrows=True, arrowsize=22, connectionstyle="arc3,rad=0.05",
        )
    edge_labels = nx.get_edge_attributes(G, "weight")
    nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_size=8)
...
if __name__ == "__main__":
    ...
    path, cost = shortest_path(g, "A", "L")
    print("Duong di:", path, "| Chi phi:", cost)
    draw_route(g, path, filename="route.png", title=f"A -> L (chi phi = {cost})")
